...
public class DataLoaderService {
    ...
    public synchronized void loadData() {
        ...
        // Step 1: Attempt to load existing vector store from the runtime data directory.
        boolean loadedFromDisk = vectorStore.loadFromFile();
        if (loadedFromDisk) {
            log.info("Vector store loaded from disk, containing {} embeddings.",
                    vectorStore.size());
        } else {
        ...
        // Step 3: Save the potentially updated vector store back to the runtime data directory if changes were made.
        if (newVectors > 0) {
            log.info("Added {} new vector embeddings from the XLSX file.", newVectors);
            try {
                vectorStore.saveToFile();
                log.info(
                        "Successfully saved updated vector store to disk with {} total embeddings.",
                        vectorStore.size());
            } catch (Exception e) {
    ...
    private void loadDataFromXlsx() {
        try {
            ...
            if (!resource.exists()) {
                log.warn(
                        "XLSX file not found in classpath at '{}'. Creating sample data if vector store is empty.",
                        XLSX_PATH);
            ...
            log.error(
                    "Error loading data from XLSX, creating sample data as fallback if needed: {}",
                    e.getMessage(), e);
            if (vectorStore.size() == 0) {
                createSampleData();
            }
        ...
        while (rows.hasNext()) {
            ...
            try {
                ...
                if (vectorStore.existsById(studentId)) {
                    log.trace(
                            "Student with ID {} already exists in vector store. Skipping embedding.",
                            studentId);
                    continue;
                }
    ...
    private List<String> createComprehensiveStudentChunks(Student student) {
        ...
        chunks.add(String.format(
                "Student Profile: %s (Student ID: %s) is enrolled in the %s program. " + "Contact email: %s. Current cumulative GPA: %.2f. " + "This student can be identified by ID %s, full name %s, or email address %s.",
                name, studentId, program, email, gpa, studentId, name, email));

        chunks.add(String.format(
                "Student ID %s belongs to %s. Name: %s. Program: %s. Email: %s. GPA: %.2f",
                studentId, name, name, program, email, gpa));

        chunks.add(String.format(
                "%s is a student with ID %s enrolled in %s program. " + "Email address: %s. Academic performance GPA: %.2f",
                name, studentId, program, email, gpa));

        if (student.getEmail() != null && !student.getEmail().trim().isEmpty()) {
            chunks.add(String.format(
                    "Email %s belongs to student %s (Student ID: %s). " + "Enrolled in %s program with GPA %.2f",
                    email, name, studentId, program, gpa));
        }

        chunks.add(String.format(
                "%s program student: %s (ID: %s). " + "Contact: %s. Academic standing GPA: %.2f",
                program, name, studentId, email, gpa));

        chunks.add(
                String.format("Academic record: %s (ID: %s) has GPA %.2f in %s program. Email: %s",
                        name, studentId, gpa, program, email));
        return chunks;
    }
    ...
    private void loadCourses(Sheet sheet) {
        ...
        while (rows.hasNext()) {
            ...
            try {
                ...
                Course course =
                        new Course(courseId, title, description, credits, instructor, capacity);
                dataStore.saveCourse(course);

                if (vectorStore.existsById(courseId)) {
                    log.trace(
                            "Course with ID {} already exists in vector store. Skipping embedding.",
                            courseId);
                    continue;
                }
    ...
    private List<String> createComprehensiveCourseChunks(Course course) {
        List<String> chunks = new ArrayList<>();
        chunks.add(String.format(
                "Course Information: %s (Course ID: %s) is taught by %s. " + "Course description: %s. Academic credit hours: %d. " + "Maximum student capacity: %d. This course can be found by ID %s, " + "title %s, or instructor name %s.",
                course.getTitle(), course.getCourseId(), course.getInstructor(),
                course.getDescription(), course.getCredits(), course.getCapacity(),
                course.getCourseId(), course.getTitle(), course.getInstructor()));

        chunks.add(String.format(
                "Course ID %s is %s. Instructor: %s. Credits: %d. Capacity: %d. " + "Description: %s",
                course.getCourseId(), course.getTitle(), course.getInstructor(),
                course.getCredits(), course.getCapacity(), course.getDescription()));

        chunks.add(String.format(
                "%s (Course ID: %s) taught by %s. %d credit hours. " + "Class capacity: %d students. Course content: %s",
                course.getTitle(), course.getCourseId(), course.getInstructor(),
                course.getCredits(), course.getCapacity(), course.getDescription()));

        chunks.add(String.format(
                "Instructor %s teaches %s (Course ID: %s). " + "Course description: %s. Credit value: %d hours. Maximum enrollment: %d",
                course.getInstructor(), course.getTitle(), course.getCourseId(),
                course.getDescription(), course.getCredits(), course.getCapacity()));

        if (course.getDescription() != null && !course.getDescription().trim().isEmpty()) {
            chunks.add(String.format(
                    "Course content for %s (ID: %s): %s. " + "Taught by %s for %d credits with capacity %d",
                    course.getTitle(), course.getCourseId(), course.getDescription(),
                    course.getInstructor(), course.getCredits(), course.getCapacity()));

            if (course.getDescription().length() > 100) {
                List<String> descriptionChunks = textProcessingService.chunkText(
                        String.format("Course %s (%s) taught by %s: %s", course.getTitle(),
                                course.getCourseId(), course.getInstructor(),
        ...
        chunks.add(String.format(
                "Academic details: %s (ID: %s) is worth %d credit hours. " + "Class size limit: %d students. Instructor: %s",
                course.getTitle(), course.getCourseId(), course.getCredits(), course.getCapacity(),
    ...
    private void loadResearch(Sheet sheet) {
        ...
        while (rows.hasNext()) {
            ...
            try {
                ...
                Research research =
                        new Research(researchId, title, abstractText, authors, year, keywords);
                dataStore.saveResearch(research);

                if (vectorStore.existsById(researchId)) {
                    log.trace(
                            "Research with ID {} already exists in vector store. Skipping embedding.",
                            researchId);
                    continue;
                }
    ...
    private List<String> createComprehensiveResearchChunks(Research research) {
        List<String> chunks = new ArrayList<>();
        chunks.add(String.format(
                "Research Paper: %s (Research ID: %s) published in %d. " + "Authors: %s. Keywords: %s. Abstract: %s. " + "This research can be found by ID %s, title %s, authors %s, or keywords %s.",
                research.getTitle(), research.getResearchId(), research.getYear(),
                research.getAuthors(), research.getKeywords(), research.getAbstractText(),
                research.getResearchId(), research.getTitle(), research.getAuthors(),
                research.getKeywords()));

        chunks.add(String.format(
                "Research ID %s: %s. Authors: %s. Publication year: %d. " + "Keywords: %s. Abstract: %s",
                research.getResearchId(), research.getTitle(), research.getAuthors(),
                research.getYear(), research.getKeywords(), research.getAbstractText()));

        chunks.add(String.format(
                "%s (Research ID: %s) by %s (%d). " + "Research keywords: %s. Summary: %s",
                research.getTitle(), research.getResearchId(), research.getAuthors(),
                research.getYear(), research.getKeywords(), research.getAbstractText()));

        chunks.add(String.format(
                "Authors %s published %s (ID: %s) in %d. " + "Keywords: %s. Abstract: %s",
                research.getAuthors(), research.getTitle(), research.getResearchId(),
                research.getYear(), research.getKeywords(), research.getAbstractText()));

        chunks.add(String.format(
                "Research keywords: %s. Paper title: %s (ID: %s). " + "Authors: %s. Year: %d. Abstract: %s",
                research.getKeywords(), research.getTitle(), research.getResearchId(),
                research.getAuthors(), research.getYear(), research.getAbstractText()));

        chunks.add(String.format(
                "%d publication: %s by %s (Research ID: %s). " + "Topics: %s. Abstract: %s",
                research.getYear(), research.getTitle(), research.getAuthors(),
                research.getResearchId(), research.getKeywords(), research.getAbstractText()));

        if (research.getAbstractText() != null && !research.getAbstractText().trim().isEmpty()) {
            chunks.add(String.format(
                    "Research abstract for %s (ID: %s) by %s: %s. " + "Published %d. Keywords: %s",
                    research.getTitle(), research.getResearchId(), research.getAuthors(),
                    research.getAbstractText(), research.getYear(), research.getKeywords()));

            if (research.getAbstractText().length() > 200) {
                List<String> abstractChunks = textProcessingService.chunkText(
                        String.format("Research %s (ID: %s) by %s (%d) abstract: %s Keywords: %s",
                                research.getTitle(), research.getResearchId(),
                                research.getAuthors(), research.getYear(),
        ...
        while (rows.hasNext()) {
            ...
            try {
                ...
                if (vectorStore.existsById(uniqueGradeId)) {
                    log.trace(
                            "Grade for student {} in course {} already exists. Skipping embedding.",
                            studentId, courseId);
                    continue;
                }
                ...
                for (int i = 0; i < chunks.size(); i++) {
                    ...
                    chunkMeta.put("id", uniqueGradeId);
    ...
    private List<String> createComprehensiveGradeChunks(Grade grade) {
        List<String> chunks = new ArrayList<>();
        chunks.add(String.format(
                "Grade Record: Student %s received grade %s in course %s. " + "Grade points: %.2f. This grade record links student ID %s with course ID %s " + "showing academic performance %s worth %.2f points.",
                grade.getStudentId(), grade.getGrade(), grade.getCourseId(), grade.getPoints(),
                grade.getStudentId(), grade.getCourseId(), grade.getGrade(), grade.getPoints()));
        ...
        chunks.add(String.format(
                ...
                grade.getCourseId(), grade.getStudentId(), grade.getGrade(), grade.getPoints(),
                grade.getCourseId()));
        ...
        chunks.add(String.format(
                "Grade points %.2f: Student %s received %s in course %s. " + "Numerical grade value and letter grade correlation",
                grade.getPoints(), grade.getStudentId(), grade.getGrade(), grade.getCourseId()));
        return chunks;
    }
    ...
    private String getCellValue(Row row, int index) {
        ...
        try {
            ...
            log.warn("Error reading cell at row {}, index {}: {}", row.getRowNum(), index,
                    e.getMessage());
            return "";
        }
    }
    ...
    private double getNumericValue(Row row, int index) {
        ...
        try {
            return switch (cell.getCellType()) {
                ...
                case STRING -> {
                    ...
                    try {
                        ...
                        log.warn("Cannot parse numeric value from string '{}' at row {}, index {}",
                                stringValue, row.getRowNum(), index);
                        yield 0.0;
                    }
                }
                case FORMULA -> {
                    try {
                        ...
                        log.warn("Cannot evaluate formula at row {}, index {}: {}", row.getRowNum(),
                                index, e.getMessage());
                        yield 0.0;
                    }
                }
            ...
            log.warn("Error reading numeric value at row {}, index {}: {}", row.getRowNum(), index,
                    e.getMessage());
            return 0.0;
        }
    }
    ...
    private void createSampleData() {
        ...
        List<Student> students = Arrays.asList(
                new Student("STU001", "John Doe", "john.doe@university.edu", "Computer Science",
                        3.75),
                new Student("STU002", "Jane Smith", "jane.smith@university.edu", "Mathematics",
                        3.85),
                new Student("STU003", "Bob Johnson", "bob.johnson@university.edu", "Physics",
                        3.60));
        ...
        List<Course> courses = Arrays.asList(new Course("CS101", "Introduction to Programming",
                        "Basic programming concepts and fundamentals", 3, "Dr. Ada Lovelace", 30),
                new Course("MATH201", "Calculus I",
                        "Differential and integral calculus with applications", 4,
                        "Dr. Isaac Newton", 25));
